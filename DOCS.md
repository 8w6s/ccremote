...
Moves the channel to an archive category before stopping the Runner or committing the closed state. This ordering prevents a failed Discord move from leaving a closed session stranded in the live channel list. When the primary category reaches Discord's 50-channel limit, ccRemote reuses or creates `{archive-name}-overflow 0001`, then `0002`, `0003`, and so on. There is no special two-category ceiling; each additional 50 archived channels allocates the next four-digit category. Legacy unsuffixed overflow categories are renamed to index `0001` without moving their children. Archive placement is serialized to prevent concurrent closes from overfilling the same category. After a successful move it stops the Runner, cancels approvals, marks the session closed, cleans eligible temporary files, and prefixes the channel. JSONL and mapping remain. A message in any mapped closed channel or `/open` can move it back to the active category and reopen it.
...
When autostart is selected, setup stages a production runtime outside the source checkout. Linux uses `$XDG_DATA_HOME/ccremote/app` or `~/.local/share/ccremote/app`; macOS uses `~/Library/Application Support/ccRemote/app`; Windows uses `%LOCALAPPDATA%\ccRemote\app`. The staged directory contains compiled `dist`, production dependencies, package metadata, and a private copy of `.env`. Setup resolves Claude Code from the interactive PATH and common per-user install locations, then stores its absolute path as `CLAUDE_BIN`; daemon startup never assumes that an interactive shell PATH is available. systemd, LaunchAgent, or Task Scheduler points to this stable runtime instead of the Git clone. Deployment uses an `app.new` staging directory and an `app.previous` rollback directory so an interrupted update does not leave a half-written application. The source checkout can therefore be deleted after successful installation without breaking the daemon, although it remains useful for updates.
